...
export default {
  ...
  getMarkersInBounds: function (bound, callback) {
    var markers = filter(this.places, (o) => {
      return bound.contains(new window.google.maps.LatLng(o.position))
    })
    ...
    each(markers, function (item, idx) {
      ...
      me.generateIcon(me.dateDiff(item.info.date), function (src) {
        ...
        if (!(item.id in checkArr)) {
          checkArr.push(item.id)
        }
        if (checkArr.length >= markers.length) {
          me.state.markers = markers
          callback()
        }
      })
    })
